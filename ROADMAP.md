...
> **Status (2026-09-24): brygge 0.2.0 (Scale) is released,** after 0.1.0 (the first release) and 0.1.1
> (Windows, and release automation). All four sources (Git, hg, SVN, CVS) decode into IR contract 0.2.0
> (see `CHANGELOG.md`). Next is **0.3.0 (Source reach)**. brygge is in **v0 development**: breaking changes made to
...
| **0.3.0** (**in progress**, RFC 013 accepted 2026-09-24) | **Source reach** | hg store path encoding completed (hashed `dh/` long paths, reserved names, `.i`/`.d` directories); SVN delta dumps (svndiff0 from `svnadmin --deltas` and `svnrdump`, with property deltas; closes `RR-svn-special-toggle`); CVS branch history (with `--reconstruct-refs`); CVS adaptive clustering windows deferred (OQ-3) | 0.2.0 released; per-item RFC amendment and security review | Each lifted limit has fixtures against real tools |
