...
> **Status (2026-09-25): brygge 0.3.0 (Source reach) is released,** after 0.2.0 (Scale), 0.1.1 (Windows,
> and release automation) and 0.1.0 (the first release). All four sources (Git, hg, SVN, CVS) decode into IR contract 0.2.0
> (see `CHANGELOG.md`). The next theme is to be planned. brygge is in **v0 development**: breaking changes made to
...
| **0.2.0** (**released 2026-09-24**) | **Scale** | Batch A: bench scenarios for Git, CVS delta chains and large SVN dumps, and a baseline on 0.1.1. Batch B, ranked by the batch A baseline: RFC 010 increments 5 (Git snapshot retention bound: 11.7× lower peak at 20k commits), 3 (CVS trunk reconstruction in one pass: 197× faster at 5k revisions per file) and 3b (CVS clustering overlap, which increment 3 revealed); increment 2 (SVN dumpstream iterator) and increment 4 (streaming writer) deferred by measurement, since every peak now tracks the IR itself (a constant multiple of content, or about 5 KiB per atom for history-heavy repositories). Batch C: maintenance (the version-free CVS reason, closing `RR-cvs-read-toctou`, an mdBook CI cache) | 0.1.1 released | Before/after measurements recorded per increment; byte-identical output for every decoder except the intended CVS reason text; four platforms green |
| **0.3.0** (**released 2026-09-25**) | **Source reach** | hg store path encoding completed (hashed `dh/` long paths, reserved names, `.i`/`.d` directories); SVN delta dumps (svndiff0 from `svnadmin --deltas` and `svnrdump`, with property deltas; closes `RR-svn-special-toggle`); CVS branch history (with `--reconstruct-refs`, nested branches included); the IR reader's canonical-order fix and one ref per Mercurial named branch (both fix every earlier release); CVS adaptive clustering windows deferred (OQ-3); CVS vendor branches after clearing deferred (OQ-6) | 0.2.0 released; per-item RFC amendment and security review | Each lifted limit has fixtures against real tools |
