...
| Date | 2026-08-31 |
| Inputs | brygge Requirements v0.1 (`brygge-01-requirements-spec-v0.1.md`) — cited as PU/NG/PR/HO/VF/ID/FA/BN/IR/UD/OQ; RFC 113 (import contract); prikk 0.27.1 reality; project rules |
...
- **CL-01 — `brygge decode <source-kind> <source-location> --ir <out>`** — read a source into an IR artifact. `<source-kind>` ∈ {`git`,`svn`,`cvs`}, each with its own honestly-scoped support (SRC-*). Decode stands alone (PU-1): it needs no target. Records the inference parameters it used into the IR (PR-5), and always writes a fidelity record for the decode half (FS-01).
...
- **CT-01 — Inputs brygge accepts:** a source repository (Git/SVN/CVS, by kind + location); an IR artifact (for `inspect`/`verify`/`encode`); inference parameters (CF-01); the declared source-side floor policy (CF-03, contents GATED). No target credentials, no network endpoints — brygge reads sources and writes files.
...
**Sequencing consequence** (RFC 113 §4a, §5): the IR obligations (IX-*) and the honesty/verification surface (FS-*, VF-*) can be internally designed next, source by source, starting with Git. The prikk encode surface (CL-03 for `prikk`) cannot be finalized past a proposal until GATED-1/2/3 land; the per-source floors cannot be finalized until GATED-4. This document deliberately leaves those open rather than guess — a confident wrong external surface would be worse than a stated gate (the requirements' own rule).

*End of External Design v0.1. Next internal design work should start where no gate blocks it: the IR's internal representation satisfying IX-*/IR-*, and the Git decoder, per RFC 113's sequencing (foundations → Git → SVN → CVS, CVS last and permitted to be lossy). SVN, CVS, and the prikk encoder past a proposal wait on the §8 gates.*
