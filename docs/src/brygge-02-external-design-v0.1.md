...
- **CT-04 — The tool's machine-readable report format** (CL-07) is a versioned contract, distinct from the IR: it is how a CI gate consumes `decode`, `inspect` and `verify` results and outcome classes. Versioned so a migration pipeline can pin it. Every key, value domain, version key and the bump rule are specified in [`reference/machine-output.md`](reference/machine-output.md). (This is a *report* schema, permitted; the *IR* schema remains the design phase's, not this document's.)
