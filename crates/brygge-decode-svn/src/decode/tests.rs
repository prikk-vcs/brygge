...
fn every_kind_of_artifact_text_this_decoder_writes_is_free_of_internal_references() {
    ...
    // only appear in particular fixtures — working-copy hints, custom properties, mergeinfo — are all
    // actually produced and therefore scanned, alongside a partial-layout flag and an empty directory.
    let mut d = DumpBuilder::new();
    d.revision(0, &[("svn:date", &date(0))]);
    d.revision(1, &[("svn:log", "props"), ("svn:date", &date(1))]);
    d.add_dir("trunk");
    d.node(
        "trunk/a.txt",
        Some("file"),
        ...
        Some(&[
            ...
            ("svn:keywords", "Id"),
            ("mycompany:custom", "x"),
            ("svn:mergeinfo", "/branches/x:1"),
        ]),
        Some(b"x\n"),
        None,
    );
    d.add_dir("trunk/emptydir");
    d.add_file("README", b"outside the layout\n");
    let opts = Options {
        reconstruct_refs: true,
        ..Options::default()
    };
    let ir = decode_dump(&d.buf, &opts).unwrap();
    let whats: Vec<&str> = ir.loss.dropped.iter().map(|x| x.what.as_str()).collect();
    assert!(
        whats.iter().any(|w| w.contains("svn:eol-style")),
        "{whats:?}"
    );
    assert!(whats.iter().any(|w| w.contains("custom")), "{whats:?}");
    assert!(whats.contains(&"svn:mergeinfo"), "{whats:?}");
    assert!(
        whats.iter().any(|w| w.starts_with("empty directories")),
        "{whats:?}"
    );
    assert!(
        ir.flags.iter().any(|f| f.what.starts_with("paths outside")),
        "{:?}",
        ir.flags
    );
    assert_no_internal_references(&ir);
